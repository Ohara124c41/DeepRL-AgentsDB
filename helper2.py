...
# This is a simple function to reshape our game buffers and create a two channel image.
def processBuffers(d_buf, l_buf, img_size):
    d = d_buf[10:-10,30:-30]
    l = l_buf[10:-10,30:-30]
    d = scipy.misc.imresize(d,[img_size, img_size])
    l = scipy.misc.imresize(l,[img_size, img_size])
    s = np.array([d,l])
    s = np.rollaxis(s, 0, 3)
    s = np.reshape(s,[np.prod(s.shape)])
    return s
...
def saveToCenter(i, rList, jList, bufferArray, summaryLength, h_size, sess, mainQN, time_per_step, img_x, img_z):
    with open('./Center/log.csv', 'a') as myfile:
        ...
        if images.shape[3] == 2:
            ...
            make_gif(l, './Center/frames/l/labels' + str(i) + '.gif', duration=len(images) * time_per_step,
                true_image=True, salience=False)
            make_gif(d, './Center/frames/image' + str(i) + '.gif', duration=len(images) * time_per_step,
                true_image=True, salience=False)
        else:
